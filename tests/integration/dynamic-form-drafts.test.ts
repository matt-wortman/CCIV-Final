...
describeIntegration('dynamic form draft integration', () => {
  ...
  it('persists drafts and rebuilds answer metadata after hydration', async () => {
    ...
    expect(loaded.data?.responses[bindingLookup.fieldCodeFor('technology.techId')]).toBe(techId);

    const nameFieldCode = bindingLookup.fieldCodeFor('technology.technologyName');
    const metadata = loaded.data?.answerMetadata?.[nameFieldCode];
  ...
  it('updates an existing draft in place when saveDraftResponse receives a draft id', async () => {
    ...
    const updatedPayload = buildSubmissionPayload(templateId, bindingLookup, techId, {
      responses: {
        [bindingLookup.fieldCodeFor('technology.technologyName')]: updatedName,
      },
    });
    ...
    const nameField = bindingLookup.fieldCodeFor('technology.technologyName');
