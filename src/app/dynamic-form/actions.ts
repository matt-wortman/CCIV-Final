...
function mergeRepeatGroupBindings(
  ...
  for (const [fieldCode, rows] of Object.entries(repeatGroups)) {
    if (!bindingMetadata[fieldCode]) {
      continue
    }
  ...
  const repeatGroupEntries = Object.entries(payload.repeatGroups).flatMap(([questionCode, rows]) =>
    Array.isArray(rows)
      ? rows.map((rowData, index) => ({
          ...
          questionRevisionId: bindingMetadata[questionCode]?.currentRevisionId ?? undefined,
        }))
      : []
  )
...
export async function loadDraftResponse(draftId: string, userId?: string) {
  try {
    ...
    const answerMetadata = buildSubmissionAnswerMetadata(
      template,
      ...
      submission.repeatGroups.map((group) => ({
        questionCode: group.questionCode,
        rowIndex: group.rowIndex,
        data: group.data,
        questionRevisionId: group.questionRevisionId ?? null,
      })),
