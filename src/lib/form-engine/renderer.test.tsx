...
describe('FormEngineProvider + DynamicFormRenderer', () => {
  ...
  it('hydrates initial data and clears answer metadata when fields change', async () => {
    const template = buildTemplate({
      sections: [
        buildSection({
          questions: [
            buildQuestion({ fieldCode: 'Q1', label: 'First Question', isRequired: true }),
          ],
        }),
      ],
    });
    ...
    const initialData = {
      responses: { Q1: 'prefill' },
      answerMetadata: {
        Q1: { status: 'STALE', answeredAt: '2025-11-01T10:00:00Z' } as AnswerStatusDetail,
      },
    };
  ...
  it('updates repeatable group data through the repeat group adapter', async () => {
    const template = buildTemplate({
      sections: [
        buildSection({
          questions: [
            buildQuestion({ fieldCode: 'RG1', label: 'Repeat Group', type: FieldType.REPEATABLE_GROUP }),
          ],
        }),
      ],
    });
    ...
    expect(Array.isArray(repeatGroups.RG1)).toBe(true);
    expect(repeatGroups.RG1).toHaveLength(1);
  });

  it('navigates between sections without losing responses', () => {
    const sectionOne = buildSection({
      questions: [buildQuestion({ fieldCode: 'Q1', label: 'Section 1 Question' })],
      order: 0,
    });
    const sectionTwo = buildSection({
      questions: [buildQuestion({ fieldCode: 'Q2', label: 'Section 2 Question' })],
      order: 1,
    });
    ...
    expect(screen.queryByTestId('field-Q2')).not.toBeInTheDocument();
  ...
  it('only re-hydrates when initial data payload changes', () => {
    const template = buildTemplate({
      sections: [
        buildSection({
          questions: [buildQuestion({ fieldCode: 'Q1', label: 'Name' })],
        }),
      ],
    });

    const initialData = { responses: { Q1: 'server-value' } };
    ...
    const nextInitialData = { responses: { Q1: 'server-replacement' } };
    rerender(
    ...
    responses = JSON.parse(screen.getByTestId('state-responses').textContent || '{}');
    expect(responses.Q1).toBe('server-replacement');
  });
  ...
  it('derives calculated scores when responses change', async () => {
    ...
    const template = buildTemplate({
      sections: [
        buildSection({
          questions: [buildQuestion({ fieldCode: 'Q1', label: 'Score Field' })],
        }),
      ],
    });
    ...
    fireEvent.change(screen.getByTestId('field-Q1'), { target: { value: 'new value' } });

    await waitFor(() => {
      expect(mockedExtractScoringInputs).toHaveBeenCalledWith({ Q1: 'new value' });
  ...
  it('triggers debounced validation through validateField helper', async () => {
    ...
    const template = buildTemplate({
      sections: [
        buildSection({
          questions: [buildQuestion({ fieldCode: 'Q1', label: 'Needs Validation', isRequired: true })],
        }),
      ],
    });
    ...
    fireEvent.change(screen.getByTestId('field-Q1'), { target: { value: 'user input' } });
