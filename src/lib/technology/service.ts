...
function buildInitialValues(
  ...
  for (const question of questions) {
    if (!question.dictionary) {
      answerMetadata[question.fieldCode] = {
        status: 'UNKNOWN',
...
export function buildSubmissionAnswerMetadata(
  ...
  for (const section of template.sections) {
    for (const question of section.questions) {
      ...
      if (question.type === FieldType.REPEATABLE_GROUP) {
        const rows = repeatGroupMap.get(question.fieldCode) ?? [];
        if (rows.length > 0) {
        ...
        result[question.fieldCode] = getAnswerStatus(question, versioned);
        continue;
      }

      const response = responseMap.get(question.fieldCode);
...
export function buildExtendedDataUpdates(
  ...
  for (const meta of Object.values(bindingMetadata)) {
    ...
    const rawValue = responses[meta.fieldCode];
