...
const makeBinding = (overrides: Partial<BindingMetadata>): BindingMetadata => ({
  fieldCode: overrides.fieldCode ?? 'FIELD',
...
describe('technology service helpers', () => {
  ...
  it('extracts binding values and partitions them by source', () => {
    const metadata = {
      TECH_NAME: makeBinding({ fieldCode: 'TECH_NAME', bindingPath: 'technology.technologyName' }),
      TRIAGE_NOTE: makeBinding({
        fieldCode: 'TRIAGE_NOTE',
        bindingPath: 'triageStage.notes',
        dataSource: DataSource.STAGE_SUPPLEMENT,
      }),
      VIABILITY_FEASIBILITY: makeBinding({
        fieldCode: 'VIABILITY_FEASIBILITY',
    ...
    const responses = {
      TECH_NAME: 'Hydration Patch',
      TRIAGE_NOTE: 'Ready for pilot',
      VIABILITY_FEASIBILITY: 'Feasible',
      EXTRA_FIELD: 'ignored',
    }
  ...
  it('builds extended data updates with answeredAt timestamps', () => {
    ...
    const metadata = {
      ...
      VIABILITY_SCORE: makeBinding({
        fieldCode: 'VIABILITY_SCORE',
